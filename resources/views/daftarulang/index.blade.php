            ...
            <div class="dt-container dt-bootstrap5 dt-empty-footer">
                <div class="row card-header flex-column flex-md-row pb-0">
                    <div
                        ...
                        <h5 class="card-title mb-0 text-md-start text-center">Data Uang Daftar Ulang</h5>
                    </div>
                    <div
                        class="d-md-flex justify-content-between align-items-center dt-layout-end col-md-auto ms-auto mt-0">
                        <div class="dt-buttons btn-group flex-wrap mb-0">
                            <x-modal modalId="tambahdu" buttonText="+ Tambah Uang Daftar Ulang"
                                title="Tambah Data Uang Daftar Ulang" url="daftarulang/store" method="post"
                                btnTutup="simpan data">
                                <div class="mb-2">
                ...
                <div class="d-md-flex justify-content-between align-items-center dt-layout-full table-responsive">
                    <div class="card table-responsive w-100 m-1">
                        <table class="table mb-0">
                            ...
                            <tbody>
                                @foreach ($daftarulang as $du)
                                    <tr>
                                        <td>{{ $du->idudu }}</td>
                                        <td>Rp.{{ number_format($du->nominal_du, 0, ',', '.') }}</td>
                                        <td>{{ $du->tahun_ajaran }}</td>
                                        <td class="d-flex gap-2">
                                            <x-modal modalId="ubahhdu{{ $du->idudu }}" buttonText="edit" title="Edit SPP"
                                                url="daftarulang/update/{{ $du->idudu }}" method="PUT" btnTutup="Ubah data">
                                                <div class="mb-2">
                                                    <label for="" class="form-label">Nominal</label>
                                                    <input type="text" class="form-control" placeholder="Masukkan Kelas"
                                                        name="nominal" value="{{ $du->nominal_du }}">
                                                </div>
                                                <div class="mb-2">
                                                    <label for="" class="form-label">Tahun Ajaran</label>
                                                    <input type="text" class="form-control" placeholder="Masukkan Tahun Ajaran"
                                                        name="tahun_ajaran" value="{{ $du->tahun_ajaran }}">
                                                </div>
                                            </x-modal>
                                            <form action="{{ route('daftarulang.destroy', $du->idudu) }}" method="POST"
